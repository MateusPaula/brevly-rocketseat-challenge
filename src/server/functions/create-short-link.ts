...
async function checkIfShortUrlExists(shortUrl: string): Promise<boolean> {
    const existingLinks = await db.select().from(schema.links).where(eq(schema.links.shortUrl, shortUrl));
    return existingLinks.length > 0;
}

export async function createLink(input: CreateShortLink): Promise<Either<ShortLinkAlreadyExistsError, { shortUrl: string, originalUrl: string, visits: number }>> {
